...
legend to name=legend-forceestimation-feedforward,
...
colorbar to name=colorbar-forceestimation-feedforward-drag,
]
% add 3D plot
\addplot3[scatter,							% scatter plot 
% (note that scatter plots have connecting lines by default)
y filter/.expression={\thisrow{alt} < 20 || ((\thisrow{eta} >= -0.087) && mod(\coordindex, 2) != 0) ? nan : y},	% throw away values at low altitude
...
table[x=alpha, y=q, z=eta, meta=Cdrag]{\data/forceestimation/cumulus/feedforward-short.dat};
